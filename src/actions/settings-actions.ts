...
export async function updateMyAccount(updates: { full_name?: string; email?: string }): Promise<ActionResult> {
  return runAction(async () => {
    ...
    if (parsed.email !== undefined && parsed.email !== u.email) {
      ...
      if (dupe) throw new AppError("Ese email ya está en uso.", "EMAIL_EN_USO");
